...
function Checkout({ cartProducts, setCartProducts }) {
  ...
  return (
    <div className="checkout">
            ...
            <div key={product.id}>
              ...
              <form onSubmit={(e) => addToCart(e, product.id)}>
                <label className="quantity-container-checkout">
                  <span className="decr" onClick={decreaseItems}>
                    -
                  </span>
                  <input
                    className="input-quantity-checkout"
                    type="text"
                    name="quantity"
                    onChange={handleQuantityInput}
                    value={quantity}
                  />
                  <span className="add" onClick={addItems}>
                    +
                  </span>
                </label>
      ...
      <div className="total">
        {total < 1 ? "Sorry, no products here." : `Total: ${total} kr`}
      </div>
      <hr className="checkout-hr"></hr>
      ...
      <div className="payment-container">
        <a
          className="payment"
          href="http://127.0.0.1:5500/planty-main/planty-main/Klarna%20Payment/payment.html"
        >
          To payment{" "}
        </a>
      </div>
      ...
      <div className="to-orderconf">
        <Link to="/OrderConf">To Order Confirmation</Link>
      </div>
    </div>
  );
}
